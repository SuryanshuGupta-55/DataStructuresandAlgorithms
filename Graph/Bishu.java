...
class Bishu {
    ...
    public void DFS(int source,int count,boolean[] vis,int[] dis){
        vis[source] = true;
        dis[source] = count;
        for(int neighbour:adj[source]){
            if(!vis[neighbour]){
                vis[neighbour] = true;
                DFS(neighbour,count+1,vis,dis);
            }
        }
    }
    public void DFS(int source,int[] a) throws IOException {
        int count = 0;
        boolean[] vis = new boolean[V];
        int[] distance = new int[V];
        DFS(source,count,vis,distance);
        /*println(Arrays.toString(distance));*/
        println( ans(distance,a)+"");
    }
    public int ans(int[] dist,int[] a){
        int smallest = Integer.MAX_VALUE;
        int ind = -1;
        for(int i = 0;i<a.length;i++){
            if(dist[a[i]] < smallest){
                smallest = dist[a[i]];
                ind = a[i];
            }
        }
